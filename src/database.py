...
from qdrant_client.models import Distance, VectorParams, PointStruct, CreateAlias, CreateAliasOperation
...
def _get_next_collection_version() -> tuple[str, str | None]:
    ...
    current_active = None
    try:
        aliases_res = qdrant_client.get_aliases()
...
def initialize_database(force_recreate: bool = False) -> str:
    ...
    if not force_recreate:
        if active_ver and qdrant_client.collection_exists(active_ver):
        ...
        if qdrant_client.collection_exists(v1_name):
